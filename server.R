...
server <- function(input, output, session) {
  ...
  data <- data.frame(read_sav(file="Backpain50_mianimate.sav"))
  
  output$downloadData <- downloadHandler(
    filename = function() {
      paste("Backpain", Sys.Date(), ".sav", sep="")
    },
  ...
  selectedData_dyn <- reactive({
    
    dat <- read_sav(file="Backpain50 MI missing.sav")
    dataset <- dat[, c("Pain", "Tampascale")]
    imp0 <- mice(data=dataset, m=1, method="mean", maxit = 0)
    pred <- imp0$predictorMatrix
    imp <- mice(data=dataset, predictorMatrix = pred, method=input$Method, 
                m=input$imp, maxit = input$niter, seed=1232)
    Imputed <- apply(imp$chainMean, 1L, c)[, "Tampascale"]
    Imputed
  }) 
  
  #values <- reactiveValues()
  #values$imp.values <- selectedData_dyn()  
  
  Iteration_start <- 1
  Iteration_end <- Iteration_start + 1
  Imputed_start <- isolate(selectedData_dyn())
  selectedData0 <- reactiveVal(data.frame(Iteration = c(Iteration_start), Imputed = c(Imputed_start[1])))
  
  iter_start <- reactiveVal(Iteration_start)
  iter_end <- reactiveVal(Iteration_end)
  
  # update our reactiveVal 
  observe({
     # every second
      isolate({
      iter_start(iter_end()) # set start time to current end time
      iter_end(iter_start() + 1) # set end time to start - 1000
      selectedData_new  <- data.frame(Iteration = iter_start(), Imputed = c(Imputed_start[iter_start()])) # create a new row for the dataframe
      selectedData0(rbind(selectedData0(),selectedData_new)) # rbind the reval_omega() and the new row, and store result in reval_omega()
      })
    if(nrow(selectedData0())<input$niter) invalidateLater(1000, session)
  })
  
  output$Imputed_Mean_Chain <- renderTable( {
        selectedData0() })
  
  selectedData2 <- eventReactive(input$gobutton,{
    if(is.null(input$niter)){
      return()
    }
    ...
    q <- sampler2(data, m, where, imp, blocks, method, visitSequence, 
                  ...
                  printFlag)
    q.means <- colMeans(q)
    q <- round(data.frame(rbind(q, q.means), row.names = c(paste("Mis", 1:13), "Means")), 2)
    q
  }) 
  
  output$table2 <- function() {
    selectedData2() %>%
      knitr::kable("html", caption = "Imputed Tampascale values at each iteration") %>%
  ...
  selectedData1 <- reactive({
    ...
    Imputed <- apply(imp$chainMean, 1L, c)[, "Tampascale"]
    Iteration <- rep(1:input$niter, input$imp)
    Impnr <- rep(1:input$imp, each=input$niter)
    
    dat_imp <- data.frame(Impnr, Iteration, Imputed)
    dat_imp
  }) 
  
  output$table1 <- function() {
    selectedData1() %>%
      knitr::kable("html", caption = "Mean of Imputed values") %>%
